<script lang="ts">
    ...
    const { data } = $props();
    let lecturers = $derived(data?.lecturers || []);
    let totalCount = $derived(data?.totalCount || 0);
    const cacheKeyName = "staff-lecturer-management";
...
>
    ...
    <div class="flex items-center justify-between mb-6 mt-2 px-1">
        ...
        >
            ...
            <span class="text-sm font-extrabold text-stone-800"
                >{totalCount}</span
            >
            <span class="text-sm text-stone-400">lecturers</span>
        </div>
    </div>
    ...
    <DataTable
        ...
        activePaginate
        matchSearchColumns={["fullName", "lecturerCode"]}
        {filters}
    >
