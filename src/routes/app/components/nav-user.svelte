<script lang="ts">
  ...
  if (user?.roles?.includes(ROLE.STUDENT)) {
    ...
    user?.roles?.includes(ROLE.ACADEMICSTAFF) ||
    user?.roles?.includes(ROLE.ADMIN)
  ) {
    userRole = "academic-staff";
  } else if (user?.roles?.includes(ROLE.MENTOR)) {
    userRole = "mentor";
  }
...
<DropdownMenu.Root>
  <DropdownMenu.Trigger>
    {#snippet child({ props })}
      ...
      >
        <Avatar.Root>
          <Avatar.Image src={user.avatarUrl ?? undefined} alt={user.fullName} />
          <Avatar.Fallback class="rounded-lg bg-amber-400 text-black font-bold">
