...
<div class="min-h-screen bg-white pt-5 font-sans">
    ...
    {:else}
        ...
        >
            <!-- Left: Cancel / Back -->
            {#if hasNoTopic || isCreatingNew}
                <Button
                    variant="ghost"
                    onclick={() => {
                        if (isCreatingNew) cancelCreateNew();
                        else goto("/app");
                    }}
                            ...
                            <div class="p-0">
                                {#each [{ label: "Title", value: topic.title }, { label: "Description", value: topic.description }, { label: "Objectives", value: topic.objectives }, { label: "Submitted At", value: formatIfDate(topic.submittedAt) }] as field, i}
                                    ...
                                    >
                                        ...
                                        {#if field.value}
                                            <span
                                                class="text-sm font-medium leading-relaxed text-gray-900 whitespace-pre-wrap"
                                                >{field.value}</span
                                            >
                                        {:else}
                                            <span
