...
<Dialog.Root {open} {onOpenChange}>
    <Dialog.Content class="max-w-2xl p-0 overflow-hidden rounded-2xl">
        ...
        >
            <div class="flex items-start justify-between gap-4">
                <div class="flex flex-col gap-1">
                    <p
                        class="text-xs font-medium text-orange-900/70 uppercase tracking-widest"
                    >
                        Group Detail
                    </p>
                    <h2 class="text-2xl font-bold text-orange-950">
                        {group?.groupName ?? "—"}
                    </h2>
                </div>
                <Badge
            ...
            <div class="mt-4 flex gap-3">
                ...
                >
                    <UsersIcon class="h-4 w-4 text-orange-900" />
                    <span class="text-sm text-orange-900/70">Members</span>
                ...
                {#if group?.topic}
                    <div
                        class="flex items-center gap-2 rounded-xl bg-black/10 px-4 py-2"
                    >
                        <BookOpenIcon class="h-4 w-4 text-orange-900" />
                        <span class="text-sm text-orange-900/70">Topic</span>
                        <span class="text-sm font-bold text-orange-950"
                            >{group.topic.title}</span
                        >
                    </div>
                {/if}
            </div>
        ...
        <!-- Body -->
        <div
            class="flex flex-col gap-5 px-6 py-5 bg-white max-h-[60vh] overflow-y-auto"
        >
            <!-- Description -->
            {#if group?.groupDescription}
                ...
                >
                    <p
                        class="text-xs font-semibold uppercase tracking-widest text-stone-400 mb-1"
                    >
                        Description
                    </p>
